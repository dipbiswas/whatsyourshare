...
import { LayoutDashboard, Users, Receipt, Settings, LogOut, DollarSign, Plane, Crown, Sun, Moon, UserCheck, Activity, Zap, Menu, X } from "lucide-react"
...
export function Sidebar({ user }: SidebarProps) {
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="hidden lg:flex h-full w-60 flex-col border-r border-border bg-muted/30 dark:bg-white/5 dark:backdrop-blur-xl">
        {/* Brand */}
      ...
      <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
        <SheetContent side="left" className="w-72 p-0 flex flex-col">
          ...
          <div className="border-t border-border p-3">
            ...
            <Button variant="ghost" size="sm" className="w-full justify-start gap-2 text-muted-foreground hover:text-foreground hover:bg-accent text-xs" onClick={toggle}>
              {theme === "dark" ? <Sun className="h-3.5 w-3.5" /> : <Moon className="h-3.5 w-3.5" />}
              {theme === "dark" ? "Light mode" : "Dark mode"}
            </Button>
            <Button variant="ghost" size="sm" className="w-full justify-start gap-2 text-muted-foreground hover:text-rose-500 hover:bg-rose-500/15 text-xs" onClick={() => signOut({ callbackUrl: "/login" })}>
              <LogOut className="h-3.5 w-3.5" />
              Sign out
            </Button>
          </div>
